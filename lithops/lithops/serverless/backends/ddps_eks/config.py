...
def load_config(config_data):
    for key in DEFAULT_CONFIG_KEYS:
        if key not in config_data['ddps_eks']:
            config_data['ddps_eks'][key] = DEFAULT_CONFIG_KEYS[key]

    if 'runtime' in config_data['ddps_eks']:
        runtime = config_data['ddps_eks']['runtime']
        registry = config_data['ddps_eks']['docker_server']
        if runtime.count('/') == 1 and registry not in runtime:
            config_data['ddps_eks']['runtime'] = f'{registry}/{runtime}'

    if config_data['ddps_eks'].get('rabbitmq_executor', False):
        config_data['ddps_eks']['amqp_url'] = config_data['rabbitmq']['amqp_url']
