...
m6id = combined_price_coremark[(combined_price_coremark['InstanceFamily'] == 'm6id')]
m6in = combined_price_coremark[(combined_price_coremark['InstanceFamily'] == 'm6in')]
m6idn = combined_price_coremark[(combined_price_coremark['InstanceFamily'] == 'm6idn')]


new_data2 = pd.concat([m6i, m6id, m6in, m6idn])
new_data2['InstanceFamily'].unique()
...
family_order = ["m6i", "m6id", "m6in", "m6idn"]

# 데이터 정렬
data = new_data2.sort_values('InstanceFamily', key=lambda x: pd.Categorical(x, categories=family_order, ordered=True))
...
ax.set_xlabel('')
ax.set_ylabel('', fontsize=font_size, fontdict={'family': font_family})
ax.set_ylim(0, 54000)
...
# ax.vlines(x=2.49, ymin=-10000, ymax=39000, color='grey', linestyle='--', lw=1, clip_on=False)
# Add text below xticks
ax.text(0.12, -0.24, 'General', transform=ax.transAxes, ha='center', va='center', fontsize=font_size-3)
ax.text(0.37, -0.24, 'Storage', transform=ax.transAxes, ha='center', va='center', fontsize=font_size-3)
ax.text(0.62, -0.24, 'Network', transform=ax.transAxes, ha='center', va='center', fontsize=font_size-3)
ax.text(0.88, -0.24, 'S+N', transform=ax.transAxes, ha='center', va='center', fontsize=font_size-3)

fig.tight_layout()

plt.savefig(f'm6-family-coremark-price.pdf', bbox_inches='tight')
plt.show()
