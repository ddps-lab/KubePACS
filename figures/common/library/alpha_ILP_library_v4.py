...
def get_aws_spot_prices(target_region='us-east-1', allow_arm=True):
    ...
    try:
        ...
        df_coremark = pd.read_csv(os.path.join(dataset_dir, '../dataset/aws_coremark_singlecore.csv'))
...
def visualize_alpha_solutions(alpha_solutions, fixed_alpha_points=None, optimal_alpha=None, verbose=False, pod_count=400, pod_cpu=2, pod_mem=8):
    ...
    # 고유한 임시 파일명 생성 (타임스탬프 + PID + 시나리오 정보)
    import time
    timestamp = int(time.time() * 1000000)  # 마이크로초 단위
    pid = os.getpid()
    temp_filename = f'alpha_performance_temp_{pod_count}_{pod_cpu}_{pod_mem}_{timestamp}_{pid}.png'
    plt.savefig(temp_filename, dpi=300, bbox_inches='tight')
    plt.close()
    
    return optimal_point, temp_filename
...
def getGoldenNodepool(file_path, pod_count, pod_cpu, pod_mem, workload_intensity="default", left=0.0, right=1, tolerance=0.01, max_iterations=20, verbose=False, scale="min", region="us-east-1"):
    ...
    # 헬퍼 함수: alpha 평가 및 best 업데이트
    def evaluate_alpha(alpha):
        nonlocal best_alpha, best_perf_per_cost, best_cost, best_performance
        nonlocal best_actual_pods, best_excess_pods, best_results
        
        alpha_values = [alpha]
        points = generate_alpha_solutions(file_path, pod_count, pod_cpu, pod_mem, alpha_values, verbose=verbose, workload_intensity=workload_intensity, scale=scale)
        explored_alphas.append(alpha)
        
        if not points:
            explored_podcount.append(0)
            return -float('inf')
        
        all_results.append(points[0])
        actual_pods = sum(result['TotalPodsOnType'] for result in points[0]['results'])
        explored_podcount.append(actual_pods)
        
        # 성능 지표: performance / (cost * actual_pods)
        performance_metric = points[0]['performance'] / (points[0]['cost'] * actual_pods)
        
        # best 업데이트
        if performance_metric > best_perf_per_cost:
            best_perf_per_cost = performance_metric
            best_alpha = alpha
            best_cost = points[0]['cost']
            best_performance = points[0]['performance']
            best_results = points[0]['results']
            best_actual_pods = actual_pods
            best_excess_pods = actual_pods - pod_count
        
        return performance_metric
    
    # 초기 구간 설정
    x1 = left + (1 - golden_ratio) * (right - left)  # ≈ 0.382
    x2 = left + golden_ratio * (right - left)         # ≈ 0.618
    ...
    while (right - left) > tolerance and iteration < max_iterations:
        if f1 >= f2:
            # x1이 더 좋은거나 같은 경우 → 구간을 [left, x2]로 축소
            right = x2
            x2 = x1
            f2 = f1
...
def getSpotVerseNodepool(filepath, pod_count, pod_cpu, pod_mem, threshold=6, unit='node', verbose=False, region='us-east-1'):
    # 데이터프레임 로드 및 전처리
    ...
    if df_filtered.empty:
        if verbose:
            print("필터링된 데이터가 없어 SpotVerse 노드풀을 생성할 수 없습니다.")
        return {} # 빈 딕셔너리 반환 또는 다른 적절한 처리
...
if __name__ == '__main__':
    ...
    # FILE_PATH = "/Users/taeyoon/Desktop/middleware2025/cmp/figure5/spotdata/msa_all/filtered/ap-northeast-1_050815.csv"
    POD_COUNT = 200
    POD_CPU = 1
    POD_MEM = 8
