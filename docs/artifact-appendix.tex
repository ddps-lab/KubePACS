...
\begin{itemize}
  ...
  \item \textbf{Outputs:} 24 figure PDFs and Tables 2 and 3 in CSV, Markdown,
    and LaTeX. Optimizer recommendations are saved as ten JSON reports.
...
The \pathcode{optimizer/} directory provides an independent locked environment
and regional CSV and AZ mapping files. Run:
\begin{lstlisting}
uv sync --locked --project optimizer
uv run --locked --project optimizer \
  python optimizer/check_optimizer.py
\end{lstlisting}
The ten scenarios in \pathcode{optimizer/scenarios.json} vary pod count,
CPU, and memory across Virginia, Oregon, Ireland, and Tokyo. They use
stored prices, availability limits, hardware specifications, benchmark
scores, and zone mappings with the existing optimizer implementation.

Use repeated \texttt{-{}-case REGION,PODS,VCPU,GIB} arguments to supply different
requirements and \texttt{-{}-output} to select a fresh output directory.
Success is \texttt{PASS: 10 local optimizer scenarios}, ten scenario JSON
...
availability limits. It also recomputes cost and capacity from the regional
CSV. Compare recommendations across regions and resource requirements.
